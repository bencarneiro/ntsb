...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        NonmotoristImpaired.objects.filter(person__accident__year=2015).delete()
        csv = pd.read_csv(f"{CSV_PATH}2015/FARS2015NationalCSV/NMImpair.csv", encoding='latin-1')
        for x in csv.index:
            ...
            primary_key = f"2015{st_case}{veh_no}{per_no}{new_impairment_id}"
            
            data_to_save = {"id": primary_key, "person": person}
            data_source = get_data_source("nonmotorist_impaired.nonmotorist_impaired", 2015)
            csv_field_name = data_source.split(".")[1]
            data_to_save['nonmotorist_impaired'] = nonmotorist_impaired_converter(csv[csv_field_name][x], 2015)
            print(data_to_save)
