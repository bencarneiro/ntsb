...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Damage.objects.filter(vehicle__accident__year=2024).delete()
        csv = pd.read_csv(f"{CSV_PATH}2024/FARS2024NationalCSV/damage.csv", encoding='latin-1')
        for x in csv.index:
            ...
            primary_key = f"2024{st_case}{veh_no}{new_damage_id}"
