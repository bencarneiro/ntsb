...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Weather.objects.filter(accident__year=2021).delete()
        csv = pd.read_csv(f"{CSV_PATH}2021/FARS2021NationalCSV/weather.csv", encoding='latin-1')
        # 2007 - 2019 
        ...
        for x in csv.index:
            ...
            if len(st_case) == 5:
                st_case = "0" + st_case
            weather_records_to_be_saved = atmospheric_condition_converter(csv['WEATHER'][x], 2021)
            for record in weather_records_to_be_saved:
                ...
                primary_key = f"2021{st_case}{new_weather_id}"
                data_to_save = {
                    "id": primary_key,
                    "accident": accident,
