...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        DriverDistracted.objects.filter(vehicle__accident__year=2011).delete()
        csv = pd.read_csv(f"{CSV_PATH}2011/FARS2011NationalCSV/DISTRACT.CSV", encoding='latin-1')
        for x in csv.index:
            ...
            primary_key = f"2011{st_case}{veh_no}{new_distraction_id}"

            data_to_save = {"vehicle": vehicle, "id": primary_key}

            data_source = get_data_source("driver_distracted.distracted_by", 2011)
