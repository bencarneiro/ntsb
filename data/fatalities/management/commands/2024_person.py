...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Person.objects.filter(accident__year=2024).delete()
        person_model_fields = [
            'person_number',
            'age',
            'sex',
        ...
        csv = pd.read_csv(f"{CSV_PATH}2024/FARS2024NationalCSV/person.csv", encoding='latin-1', low_memory=False).fillna(0)
        for x in csv.index:
            ...
            while len(per_no) < 3:
                per_no = "0" + per_no
            primary_key = f"2024{st_case}{veh_no}{per_no}"
            
            accident = Accident.objects.get(year=2024, st_case=csv['ST_CASE'][x])
            data_to_save = {
                "id": primary_key,
                "accident": accident
            }
            ...
            for model_field_name in person_model_fields:
                data_source = get_data_source("person." + model_field_name, 2024)
