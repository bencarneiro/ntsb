...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        InjuryPerson.objects.filter(injury_accident__state_id=48, injury_accident__dt__year=2022).delete()
        InjuryVehicle.objects.filter(injury_accident__state_id=48, injury_accident__dt__year=2022).delete()
        InjuryAccident.objects.filter(state_id=48, dt__year=2022).delete()
        data = pd.read_csv(f"{TEXAS_PATH}2022_texas_1.csv")
        crash_id = None
        for x in data.index:
            print(x)
