...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        VehicleSequenceOfEvents.objects.filter(vehicle__accident__year=2005).delete()
        VehicleSequenceOfEvents.objects.filter(parked_vehicle__accident__year=2005).delete()
        ...
        for x in csv.index:
            ...
            while len(veh_no) < 3:
                veh_no = "0" + veh_no
            vehicle = Vehicle.objects.get(accident__year=2005, accident__st_case=csv['ST_CASE'][x], vehicle_number=csv['VEH_NO'][x])
            for event in ['SEQ1', "SEQ2", "SEQ3", "SEQ4", "SEQ5", "SEQ6"]:
                ...
                primary_key = f"2005{st_case}{veh_no}{new_event_id}"
                soe = soe_converter(csv[event][x], 2005)
                data_to_save = {
                    "id": primary_key,
                    "vehicle": vehicle,
        ...
        for x in csv.index:
            ...
            parked_vehicle = ParkedVehicle.objects.get(accident__year=2005, accident__st_case=csv['ST_CASE'][x], vehicle_number=csv['VEH_NO'][x])
            for event in ['SEQ1', "SEQ2", "SEQ3", "SEQ4", "SEQ5", "SEQ6"]:
                st_case = str(csv['ST_CASE'][x])
                if len(st_case) == 5:
                    st_case = "0" + st_case
