...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        VehicleFactor.objects.filter(vehicle__accident__year=2014).delete()
        csv = pd.read_csv(f"{CSV_PATH}2014/FARS2014NationalCSV/FACTOR.CSV", encoding='latin-1')
        for x in csv.index:
            ...
            data_source = get_data_source("vehicle_factor.contributing_cause", 2014)
            csv_field_name = data_source.split(".")[1]
            data_to_save['contributing_cause'] = vehicle_factor_converter(csv[csv_field_name][x],2014)
            print(data_to_save)
