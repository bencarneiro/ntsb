...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Violation.objects.filter(vehicle__accident__year=2015).delete()
        csv = pd.read_csv(f"{CSV_PATH}2015/FARS2015NationalCSV/Violatn.csv", encoding='latin-1')
        for x in csv.index:
            ...
            primary_key = f"2015{st_case}{veh_no}{new_violation_id}"

            data_to_save = {"vehicle": vehicle, "id": primary_key}
            data_source = get_data_source("violation.moving_violation", 2015)
            csv_field_name = data_source.split(".")[1]
            data_to_save['moving_violation'] = violation_converter(csv[csv_field_name][x], 2015)
            print(data_to_save)
