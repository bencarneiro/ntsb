...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        DriverRelatedFactor.objects.filter(vehicle__accident__year=1994).delete()
        ...
        for x in csv.index:
            ...
            for factor in ["DR_CF1", "DR_CF2", "DR_CF3"]:
                ...
                # data_source = get_data_source("driver_related_factor.driver_related_factor", 1994)
                # csv_field_name = data_source.split(".")[1]
                factor_code = driver_related_factor_converter(csv[factor][x], 1994)
                if factor_code:
