...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Vision.objects.filter(vehicle__accident__year=2011).delete()
        csv = pd.read_csv(f"{CSV_PATH}2011/FARS2011NationalCSV/VISION.CSV", encoding='latin-1')
        for x in csv.index:
            ...
            primary_key = f"2011{st_case}{veh_no}{new_vision_id}"

            data_to_save = {"vehicle": vehicle, "id": primary_key}
            data_source = get_data_source("vision.visibility", 2011)
            csv_field_name = data_source.split(".")[1]
            data_to_save['visibility'] = visibility_converter(csv[csv_field_name][x], 2011)
            print(data_to_save)
