...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        DriverRelatedFactor.objects.filter(vehicle__accident__year=2015).delete()
        ...
        for x in csv.index:
            ...
            for factor in ["DR_SF1", "DR_SF2", "DR_SF3", "DR_SF4"]:
                ...
                # data_source = get_data_source("driver_related_factor.driver_related_factor", 2015)
                # csv_field_name = data_source.split(".")[1]
                factor_code = driver_related_factor_converter(csv[factor][x], 2015)
                if factor_code:
                    data_to_save = {
                        "id": primary_key,
                        "vehicle": vehicle,
