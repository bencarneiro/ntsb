...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        VehicleRelatedFactor.objects.filter(vehicle__accident__year=2015).delete()
        csv = pd.read_csv(f"{CSV_PATH}2015/FARS2015NationalCSV/vehicle.csv", encoding='latin-1')
        for x in csv.index:
            ...
            for factor in ["VEH_SC1", "VEH_SC2"]:
                ...
                # data_source = get_data_source("vehicle_related_factor.vehicle_related_factor", 2015)
                # csv_field_name = data_source.split(".")[1]
                factor_code = vehicle_related_factor_converter(csv[factor][x], 2015)
                if factor_code:
                    data_to_save = {
                        "id": primary_key,
                        "vehicle": vehicle,
