...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        DriverDistracted.objects.filter(vehicle__accident__year=2020).delete()
        csv = pd.read_csv(f"{CSV_PATH}2020/FARS2020NationalCSV/distract.csv", encoding='latin-1')
        for x in csv.index:
            ...
            primary_key = f"2020{st_case}{veh_no}{new_distraction_id}"

            data_to_save = {"vehicle": vehicle, "id": primary_key}

            data_source = get_data_source("driver_distracted.distracted_by", 2020)
