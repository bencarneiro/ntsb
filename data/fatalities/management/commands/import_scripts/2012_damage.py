...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Damage.objects.filter(vehicle__accident__year=2012).delete()
        csv = pd.read_csv(f"{CSV_PATH}2012/FARS2012NationalCSV/DAMAGE.CSV", encoding='latin-1')
        for x in csv.index:
            ...
            primary_key = f"2012{st_case}{veh_no}{new_damage_id}"
            


            data_to_save = {"vehicle": vehicle, "id": primary_key}
            data_source = get_data_source("damage.area_of_impact", 2012)
