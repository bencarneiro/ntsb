...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        NonmotoristDistracted.objects.filter(person__accident__year=2023).delete()
        csv = pd.read_csv(f"{CSV_PATH}2023/FARS2023NationalCSV/nmdistract.csv", encoding='latin-1')
        for x in csv.index:
            ...
            primary_key = f"2023{st_case}{veh_no}{per_no}{new_distraction_id}"
