...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        ParkedVehicle.objects.filter(accident__year=2020).delete()
        ...
        csv = pd.read_csv(f"{CSV_PATH}2020/FARS2020NationalCSV/parkwork.csv", encoding='latin-1')
        for x in csv.index:
            ...
            for model_field_name in vehicle_model_fields:
                data_source = get_data_source("parked_vehicle." + model_field_name, 2020)
                if data_source:
