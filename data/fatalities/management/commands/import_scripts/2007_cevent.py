...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        VehicleSequenceOfEvents.objects.filter(vehicle__accident__year=2007).delete()
        VehicleSequenceOfEvents.objects.filter(parked_vehicle__accident__year=2007).delete()

        # csv = pd.read_csv(f"{CSV_PATH}2007/FARS2007NationalCSV/CEVENT.CSV", encoding='latin-1')
        csv = pd.read_csv(f"{CSV_PATH}2007/FARS2007NationalCSV/VEHICLE.CSV", encoding='latin-1')
        for x in csv.index:
            veh_no = str(csv['VEH_NO'][x])
            while len(veh_no) < 3:
                veh_no = "0" + veh_no
            vehicle = Vehicle.objects.get(accident__year=2007, accident__st_case=csv['ST_CASE'][x], vehicle_number=csv['VEH_NO'][x])
            for event in ['SEQ1', "SEQ2", "SEQ3", "SEQ4", "SEQ5", "SEQ6"]:
                ...
                primary_key = f"2007{st_case}{veh_no}{new_event_id}"
                soe = soe_converter(csv[event][x], 2007)
                data_to_save = {
                    "id": primary_key,
                    "vehicle": vehicle,
