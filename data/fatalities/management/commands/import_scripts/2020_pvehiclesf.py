...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        ParkedVehicleRelatedFactor.objects.filter(parked_vehicle__accident__year=2020).delete()
        csv = pd.read_csv(f"{CSV_PATH}2020/FARS2020NationalCSV/pvehiclesf.csv", encoding='latin-1')
        for x in csv.index:
            print(csv['ST_CASE'][x])
            print(csv['VEH_NO'][x])
            parked_vehicle = ParkedVehicle.objects.get(accident__year=2020, accident__st_case=csv['ST_CASE'][x], vehicle_number=csv['VEH_NO'][x])
            ...
            data_source = get_data_source("parked_vehicle_related_factor.vehicle_related_factor", 2020)
            csv_field_name = data_source.split(".")[1]
            data_to_save = {
                "id": primary_key,
                "parked_vehicle": parked_vehicle,
                "parked_vehicle_related_factor": parked_vehicle_related_factor_converter(csv[csv_field_name][x], 2020)
            }
            print(data_to_save)
