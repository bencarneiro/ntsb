...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        PersonRelatedFactor.objects.filter(person__accident__year=2021).delete()
        csv = pd.read_csv(f"{CSV_PATH}2021/FARS2021NationalCSV/personrf.csv", encoding='latin-1')
        for x in csv.index:
            if csv['VEH_NO'][x] == 0:
            ...
            data_source = get_data_source("person_related_factor.person_related_factor", 2021)
            csv_field_name = data_source.split(".")[1]
            data_to_save['person_related_factor'] = person_related_factor_converter(csv[csv_field_name][x], 2021)
            print(data_to_save)
