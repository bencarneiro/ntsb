...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):   
        ...
        vehicles = []
        persons = []     
        InjuryPerson.objects.filter(injury_accident__state_id=26, injury_accident__dt__year=2008).delete()
        InjuryVehicle.objects.filter(injury_accident__state_id=26, injury_accident__dt__year=2008).delete()
        InjuryAccident.objects.filter(state_id=26, dt__year=2008).delete()
        year = 2008
        start = 1
        end = 90
        ...
        while start <= total:
            ...
            for x in data.index:
                ...
                if int(data['Crash Instance'][x]) not in saved_crashes:
                    ...
                    datestring = f"2008-{month}-{str(data['Crash Day'][x]).zfill(2)}T{hour}:00:00"
        ...
        new_crashes = InjuryAccident.objects.filter(state_id=26, dt__year=2008)
