...
# multiple sources before 1979

class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        VehicleRelatedFactor.objects.filter(vehicle__accident__year=1979).delete()
        ...
        for x in csv.index:
            if x % 1000 == 999:
                ...
                bulk_data_upload = []
                print("WE HIT THE DB")
            vehicle = Vehicle.objects.get(accident__year=1979, accident__st_case=csv['ST_CASE'][x], vehicle_number=csv['VEH_NO'][x])
            ...
            for factor in ["VEH_CF1", "VEH_CF2"]:
                ...
                primary_key = f"1979{st_case}{veh_no}{new_factor_id}"
                # data_source = get_data_source("vehicle_related_factor.vehicle_related_factor", 1979)
                # csv_field_name = data_source.split(".")[1]
                factor_code = vehicle_related_factor_converter(csv[factor][x], 1979)
                if factor_code:
