...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Weather.objects.filter(accident__year=2007).delete()
        ...
        for x in csv.index:
            ...
            if len(st_case) == 5:
                st_case = "0" + st_case
            weather_records_to_be_saved = atmospheric_condition_converter(csv['WEATHER1'][x], 2007) + atmospheric_condition_converter(csv['WEATHER2'][x], 2007)
            for record in weather_records_to_be_saved:
                if record:
                    ...
                    primary_key = f"2007{st_case}{new_weather_id}"
                    data_to_save = {
                        "id": primary_key,
                        "accident": accident,
