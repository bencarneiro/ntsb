...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        NonmotoristPriorAction.objects.filter(person__accident__year=2021).delete()
        csv = pd.read_csv(f"{CSV_PATH}2021/FARS2021NationalCSV/nmprior.csv", encoding='latin-1')
        for x in csv.index:
            ...
            primary_key = f"2021{st_case}{veh_no}{per_no}{new_factor_id}"
            data_to_save = {"id": primary_key, "person": person}
            data_source = get_data_source("nonmotorist_prior_action.nonmotorist_prior_action", 2021)
