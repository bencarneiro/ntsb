...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Person.objects.filter(accident__year=2023).delete()
        person_model_fields = [
            'person_number',
            'age',
            'sex',
        ...
        csv = pd.read_csv(f"{CSV_PATH}2023/FARS2023NationalCSV/person.csv", encoding='latin-1', low_memory=False).fillna(0)
        for x in csv.index:
            ...
            while len(per_no) < 3:
                per_no = "0" + per_no
            primary_key = f"2023{st_case}{veh_no}{per_no}"
            ...
            for model_field_name in person_model_fields:
                data_source = get_data_source("person." + model_field_name, 2023)
