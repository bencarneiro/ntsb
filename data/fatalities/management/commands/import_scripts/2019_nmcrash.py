...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        NonmotoristContributingCircumstance.objects.filter(person__accident__year=2019).delete()
        csv = pd.read_csv(f"{CSV_PATH}2019/FARS2019NationalCSV/NMCrash.CSV", encoding='latin-1')
        for x in csv.index:
            ...
            data_source = get_data_source("nonmotorist_contributing_circumstance.nonmotorist_contributing_circumstance", 2019)
