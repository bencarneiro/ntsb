...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        DriverRelatedFactor.objects.filter(vehicle__accident__year=2020).delete()
        csv = pd.read_csv(f"{CSV_PATH}2020/FARS2020NationalCSV/driverrf.csv", encoding='latin-1')
        for x in csv.index:
            ...
            data_source = get_data_source("driver_related_factor.driver_related_factor", 2020)
            csv_field_name = data_source.split(".")[1]
            data_to_save = {
                "id": primary_key,
                "vehicle": vehicle,
                "driver_related_factor": driver_related_factor_converter(csv[csv_field_name][x], 2020)
            }
            print(data_to_save)
