...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        Accident.objects.filter(year=1983).delete()
        missing_counties = []
        ...
        csv = pd.read_csv(f"{CSV_PATH}1983/ACCIDENT.CSV", encoding='latin-1')
        csv = csv.replace({np.nan: None})
        for x in csv.index:
            st_case = str(csv['ST_CASE'][x])
            if len(st_case) == 5:
                st_case = "0" + st_case
            primary_key = f"1983{st_case}"
            ...
            for model_field_name in accident_model_fields:
                data_source = get_data_source("accident." + model_field_name, 1983)
                if data_source:
        ...
        for a in Accident.objects.filter(year=1983).order_by("st_case"):
