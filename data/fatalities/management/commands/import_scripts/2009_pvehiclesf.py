...
# multiple sources before 2009

class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        ParkedVehicleRelatedFactor.objects.filter(parked_vehicle__accident__year=2009).delete()
        csv = pd.read_csv(f"{CSV_PATH}2009/FARS2009NationalCSV/VEHNIT.CSV", encoding='latin-1')
        for x in csv.index:
            parked_vehicle = ParkedVehicle.objects.get(accident__year=2009, accident__st_case=csv['ST_CASE'][x], vehicle_number=csv['VEH_NO'][x])
            ...
            for factor in ["VEH_CF1", "VEH_CF2"]:
                ...
                primary_key = f"2009{st_case}{veh_no}{new_factor_id}"
                # data_source = get_data_source("parked_vehicle_related_factor.vehicle_related_factor", 2009)
                # csv_field_name = data_source.split(".")[1]
                factor_code = parked_vehicle_related_factor_converter(csv[factor][x], 2009)
                if factor_code:
                    data_to_save = {
                        "id": primary_key,
