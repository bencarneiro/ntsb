...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Vehicle.objects.filter(accident__year=1978).delete()
        ...
        csv = pd.read_csv(f"{CSV_PATH}1978/VEHICLE.CSV", encoding='latin-1')
        csv = csv.replace({np.nan: None})
        accident_csv = pd.read_csv(f"{CSV_PATH}1978/ACCIDENT.CSV", encoding='latin-1')
        for x in csv.index:
            ...
            while len(veh_no) < 3:
                veh_no = "0" + veh_no
            primary_key = f"1978{st_case}{veh_no}"
            accident = Accident.objects.get(id=int(f"1978{st_case}"))
            data_to_save = {
                "id": primary_key,
                "accident": accident,
            ...
            for vehicle_model_field_name in vehicle_model_fields:
                data_source = get_data_source("vehicle." + vehicle_model_field_name, 1978)
                if data_source:
                    csv_field_name = data_source.split(".")[1]
                    data_converter_function = FARS_DATA_CONVERTERS["vehicle." + vehicle_model_field_name]
                    data_to_save[vehicle_model_field_name] = data_converter_function(csv[csv_field_name][x], 1978)

            for accident_model_field_name in accident_model_fields:
                data_source = get_data_source("vehicle." + accident_model_field_name, 1978)
                if data_source:
                    csv_field_name = data_source.split(".")[1]
                    data_converter_function = FARS_DATA_CONVERTERS["vehicle." + accident_model_field_name]
                    data_to_save[accident_model_field_name] = data_converter_function(accident_temp_df[csv_field_name][0], 1978)


            speeding_related = None
            speeding_related = speeding_related_converter(csv["VIOL_CHG"][x], "violation", 1978)
            if not speeding_related:
                for factor in ["DR_CF1", "DR_CF2", "DR_CF3"]:
                    speeding_related = speeding_related_converter(csv[factor][x], "drf", 1978)
