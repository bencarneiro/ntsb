...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Violation.objects.filter(vehicle__accident__year=1981).delete()
        ...
        for x in csv.index:
            ...
            primary_key = f"1981{st_case}{veh_no}001"
            violation_code = violation_converter(csv["VIOL_CHG"][x], 1981)
            if violation_code:
