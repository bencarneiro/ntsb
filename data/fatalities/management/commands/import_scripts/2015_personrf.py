...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        PersonRelatedFactor.objects.filter(person__accident__year=2015).delete()
        csv = pd.read_csv(f"{CSV_PATH}2015/FARS2015NationalCSV/person.csv", encoding='latin-1')
        for x in csv.index:
            if csv['VEH_NO'][x] == 0:
                ...
                person = Person.objects.get(Q(accident__year=2015), Q(person_number=csv['PER_NO'][x]),  Q(accident__st_case=csv['ST_CASE'][x]), Q(vehicle__vehicle_number=csv['VEH_NO'][x])|Q(parked_vehicle__vehicle_number=csv['VEH_NO'][x]))
            ...
            for factor in ["P_SF1", "P_SF2", "P_SF3"]:
                ...
                # data_source = get_data_source("person_related_factor.person_related_factor", 2015)
                # csv_field_name = data_source.split(".")[1]
                factor_code = person_related_factor_converter(csv[factor][x], 2015)
                if factor_code:
