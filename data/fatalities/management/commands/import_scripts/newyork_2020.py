...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        InjuryPerson.objects.filter(injury_accident__state_id=36, injury_accident__dt__year=2020).delete()
        InjuryVehicle.objects.filter(injury_accident__state_id=36, injury_accident__dt__year=2020).delete()
        InjuryAccident.objects.filter(state_id=36, dt__year=2020).delete()

        crashes_path = f"{NEW_YORK_PATH}crash_2020.csv"
        vehicle_path = f"{NEW_YORK_PATH}vehicle_2020.csv"
        person_path = f"{NEW_YORK_PATH}person_2020.csv"
