...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Weather.objects.filter(accident__year=1987).delete()
        ...
        for x in csv.index:
            ...
            if len(st_case) == 5:
                st_case = "0" + st_case
            weather_records_to_be_saved = atmospheric_condition_converter(csv['WEATHER'][x], 1987)
            ...
            for record in weather_records_to_be_saved:
                if record:
                    ...
                    primary_key = f"1987{st_case}{new_weather_id}"
