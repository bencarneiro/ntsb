...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Vision.objects.filter(vehicle__accident__year=2009).delete()
        ...
        for x in csv.index:
            ...
            for code in ["D_VISION1", "D_VISION2", "D_VISION3"]:
                ...
                vision = visibility_converter(csv[code][x], 2009)
                if vision:
                    data_to_save = {"vehicle": vehicle, "id": primary_key, "visibility": vision}
                    print(data_to_save)
