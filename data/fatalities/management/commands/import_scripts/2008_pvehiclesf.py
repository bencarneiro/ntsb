...
# multiple sources before 2008

class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        ParkedVehicleRelatedFactor.objects.filter(parked_vehicle__accident__year=2008).delete()
        ...
        for x in csv.index:
            ...
            for factor in ["VEH_CF1", "VEH_CF2"]:
                ...
                primary_key = f"2008{st_case}{veh_no}{new_factor_id}"
                # data_source = get_data_source("parked_vehicle_related_factor.vehicle_related_factor", 2008)
                # csv_field_name = data_source.split(".")[1]
                factor_code = parked_vehicle_related_factor_converter(csv[factor][x], 2008)
                if factor_code:
                    data_to_save = {
                        "id": primary_key,
