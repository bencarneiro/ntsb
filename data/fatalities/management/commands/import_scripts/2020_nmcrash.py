...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        NonmotoristContributingCircumstance.objects.filter(person__accident__year=2020).delete()
        csv = pd.read_csv(f"{CSV_PATH}2020/FARS2020NationalCSV/nmcrash.csv", encoding='latin-1')
        for x in csv.index:
            ...
            data_source = get_data_source("nonmotorist_contributing_circumstance.nonmotorist_contributing_circumstance", 2020)
