...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        Accident.objects.filter(year=2008).delete()
        ...
        for x in csv.index:
            ...
            for model_field_name in accident_model_fields:
                data_source = get_data_source("accident." + model_field_name, 2008)
                if data_source:
        ...
        for a in Accident.objects.filter(year=2008).order_by("st_case"):
