...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        CrashEvent.objects.filter(accident__year=2016).delete()
        model_fields =  [ "crash_event_number",
             "area_of_impact_1",
             "area_of_impact_2",
             "sequence_of_events"]

        csv = pd.read_csv(f"{CSV_PATH}2016/CEvent.CSV", encoding='latin-1')
        ...
        for x in csv.index:
            ...
            primary_key = f"2016{st_case}{new_event_id}"
            ...
            for model_field_name in model_fields:
                data_source = get_data_source("crash_event." + model_field_name, 2016)
                if data_source:
