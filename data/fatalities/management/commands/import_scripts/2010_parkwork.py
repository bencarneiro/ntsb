...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        ParkedVehicle.objects.filter(accident__year=2010).delete()
        ...
        for x in csv.index:
            ...
            while len(veh_no) < 3:
                veh_no = "0" + veh_no
            primary_key = f"2010{st_case}{veh_no}"
            ...
            for model_field_name in vehicle_model_fields:
                data_source = get_data_source("parked_vehicle." + model_field_name, 2010)
                if data_source:
