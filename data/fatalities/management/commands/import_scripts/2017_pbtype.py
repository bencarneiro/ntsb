...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        PedestrianType.objects.filter(person__accident__year=2017).delete()
        model_fields = [
            'age',
            'sex',
            'person_type',
        ...
        csv = pd.read_csv(f"{CSV_PATH}2017/PBType.CSV", encoding='latin-1')
        for x in csv.index:
            ...
            if csv['VEH_NO'][x] == 0:
                ...
                person = Person.objects.get(Q(accident__year=2017), Q(person_number=csv['PER_NO'][x]),  Q(accident__st_case=csv['ST_CASE'][x]), Q(vehicle__vehicle_number=csv['VEH_NO'][x])|Q(parked_vehicle__vehicle_number=csv['VEH_NO'][x]))
            data_to_save = {
                "id": person.id,
                "person": person,
            }
            for model_field_name in model_fields:
                data_source = get_data_source("pedestrian_type." + model_field_name, 2017)
                if data_source:
