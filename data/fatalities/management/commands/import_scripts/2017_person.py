...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Person.objects.filter(accident__year=2017).delete()
        person_model_fields = [
            'person_number',
            'age',
            'sex',
        ...
        csv = pd.read_csv(f"{CSV_PATH}2017/Person.CSV", encoding='latin-1', low_memory=False).fillna(0)
        for x in csv.index:
            ...
            while len(per_no) < 3:
                per_no = "0" + per_no
            primary_key = f"2017{st_case}{veh_no}{per_no}"
            
            accident = Accident.objects.get(year=2017, st_case=csv['ST_CASE'][x])
            data_to_save = {
                "id": primary_key,
                "accident": accident
            }
            ...
            for model_field_name in person_model_fields:
                data_source = get_data_source("person." + model_field_name, 2017)
                if data_source:
                    csv_field_name = data_source.split(".")[1]
                    data_converter_function = FARS_DATA_CONVERTERS["person." + model_field_name]
                    data_to_save[model_field_name] = data_converter_function(csv[csv_field_name][x], 2017)
                    # this will need to be fixed in 2017
            data_to_save['helmet_misuse'] = helmet_misuse_converter(csv['REST_USE'][x], csv['REST_MIS'][x], None, 2017)
