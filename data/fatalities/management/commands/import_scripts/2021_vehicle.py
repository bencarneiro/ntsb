...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Vehicle.objects.filter(accident__year=2021).delete()
        ...
        csv = pd.read_csv(f"{CSV_PATH}2021/FARS2021NationalCSV/vehicle.csv", encoding='latin-1')
        for x in csv.index:
            ...
            accident = Accident.objects.get(id=int(f"2021{st_case}"))
            data_to_save = {
                "id": primary_key,
                "accident": accident,
