...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Race.objects.filter(person__accident__year=2021).delete()
        csv = pd.read_csv(f"{CSV_PATH}2021/FARS2021NationalCSV/race.csv", encoding='latin-1')
        for x in csv.index:
            if csv['VEH_NO'][x] == 0:
            ...
            primary_key = f"2021{st_case}{veh_no}{per_no}{new_race_id}"
