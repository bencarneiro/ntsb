...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        InjuryPerson.objects.filter(injury_accident__state_id=8, injury_accident__dt__year=2008).delete()
        InjuryVehicle.objects.filter(injury_accident__state_id=8, injury_accident__dt__year=2008).delete()
        InjuryAccident.objects.filter(state_id=8, dt__year=2008).delete()
        data = pd.read_excel(f"{COLORADO_PATH}CDOTRM_CD_Crash_Listing_-_2008.xlsx")
        data = data[(data['INJURY 04']>0) | (data['INJURY 03']>0)]

        for x in data.index:
            state_accident_id = "2008" + str(x).zfill(8)
