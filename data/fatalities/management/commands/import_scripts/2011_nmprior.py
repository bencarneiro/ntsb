...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        NonmotoristPriorAction.objects.filter(person__accident__year=2011).delete()
        csv = pd.read_csv(f"{CSV_PATH}2011/FARS2011NationalCSV/NMPRIOR.CSV", encoding='latin-1')
        for x in csv.index:
            ...
            data_source = get_data_source("nonmotorist_prior_action.nonmotorist_prior_action", 2011)
