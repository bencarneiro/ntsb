...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        CrashRelatedFactors.objects.filter(accident__year=2021).delete()
        csv = pd.read_csv(f"{CSV_PATH}2021/FARS2021NationalCSV/crashrf.csv", encoding='latin-1')
        for x in csv.index:
            ...
            primary_key = f"2021{st_case}{new_factor_id}"

            data_to_save = {
                "id": primary_key,
                "accident": accident,
                "crash_related_factor": crash_related_factor_converter(csv['CRASHRF'][x], 2021)
            }
            print(data_to_save)
