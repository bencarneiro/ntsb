...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Vehicle.objects.filter(accident__year=2015).delete()
        ...
        for x in csv.index:
            ...
            while len(veh_no) < 3:
                veh_no = "0" + veh_no
            primary_key = f"2015{st_case}{veh_no}"
            
            accident = Accident.objects.get(id=int(f"2015{st_case}"))
            data_to_save = {
                "id": primary_key,
                "accident": accident,
            ...
            for model_field_name in vehicle_model_fields:
                data_source = get_data_source("vehicle." + model_field_name, 2015)
                if data_source:
                    ...
                    # fix this in 2008
            data_to_save['speeding_related'] = speeding_related_converter(csv['SPEEDREL'][x], "speeding", 2015)
            print(data_to_save)
