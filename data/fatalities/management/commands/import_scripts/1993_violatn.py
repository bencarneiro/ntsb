...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Violation.objects.filter(vehicle__accident__year=1993).delete()
        ...
        for x in csv.index:
            ...
            primary_key = f"1993{st_case}{veh_no}001"
            violation_code = violation_converter(csv["VIOL_CHG"][x], 1993)
            if violation_code:
