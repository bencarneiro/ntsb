...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        PersonRelatedFactor.objects.filter(person__accident__year=1976).delete()
        csv = pd.read_csv(f"{CSV_PATH}1976/PERSON.CSV", encoding='latin-1')
        bulk_data_upload=[]
        for x in csv.index:
            if x % 1000 == 999:
                ...
                print("WE HIT THE DB")
            if csv['VEH_NO'][x] == 0:
                person = Person.objects.get(accident__year=1976, person_number=csv['PER_NO'][x], accident__st_case=csv['ST_CASE'][x], vehicle__vehicle_number__isnull=True, parked_vehicle__vehicle_number__isnull=True)
            else:
                person = Person.objects.get(Q(accident__year=1976), Q(person_number=csv['PER_NO'][x]),  Q(accident__st_case=csv['ST_CASE'][x]), Q(vehicle__vehicle_number=csv['VEH_NO'][x])|Q(parked_vehicle__vehicle_number=csv['VEH_NO'][x]))
            ...
            for factor in ["P_CF1", "P_CF2", "P_CF3"]:
                ...
                primary_key = f"1976{st_case}{veh_no}{per_no}{new_factor_id}"
                
                # data_to_save = {"id": primary_key, "person": person}
                # factor_code = person_related_factor_converter(csv[factor][x], 1976)
                # if factor_code:
