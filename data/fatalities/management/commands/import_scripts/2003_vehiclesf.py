...
# multiple sources before 2003

class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        VehicleRelatedFactor.objects.filter(vehicle__accident__year=2003).delete()
        csv = pd.read_csv(f"{CSV_PATH}2003/VEHICLE.CSV", encoding='latin-1')
        for x in csv.index:
            vehicle = Vehicle.objects.get(accident__year=2003, accident__st_case=csv['ST_CASE'][x], vehicle_number=csv['VEH_NO'][x])
            for factor in ["VEH_CF1", "VEH_CF2"]:
                ...
                primary_key = f"2003{st_case}{veh_no}{new_factor_id}"
                # data_source = get_data_source("vehicle_related_factor.vehicle_related_factor", 2003)
                # csv_field_name = data_source.split(".")[1]
                factor_code = vehicle_related_factor_converter(csv[factor][x], 2003)
                if factor_code:
                    data_to_save = {
                        "id": primary_key,
                        "vehicle": vehicle,
