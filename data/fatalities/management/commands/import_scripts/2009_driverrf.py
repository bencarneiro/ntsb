...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        DriverRelatedFactor.objects.filter(vehicle__accident__year=2009).delete()
        ...
        for x in csv.index:
            ...
            for factor in ["DR_CF1", "DR_CF2", "DR_CF3", "DR_CF4"]:
                ...
                # data_source = get_data_source("driver_related_factor.driver_related_factor", 2009)
                # csv_field_name = data_source.split(".")[1]
                factor_code = driver_related_factor_converter(csv[factor][x], 2009)
                if factor_code:
                    data_to_save = {
                        "id": primary_key,
                        "vehicle": vehicle,
