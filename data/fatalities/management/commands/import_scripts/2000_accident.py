...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        Accident.objects.filter(year=2000).delete()
        ...
        for x in csv.index:
            ...
            for model_field_name in accident_model_fields:
                data_source = get_data_source("accident." + model_field_name, 2000)
                if data_source:
        ...
        for a in Accident.objects.filter(year=2000).order_by("st_case"):
