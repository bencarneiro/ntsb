...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Violation.objects.filter(vehicle__accident__year=1986).delete()
        ...
        for x in csv.index:
            ...
            primary_key = f"1986{st_case}{veh_no}001"
            violation_code = violation_converter(csv["VIOL_CHG"][x], 1986)
            if violation_code:
