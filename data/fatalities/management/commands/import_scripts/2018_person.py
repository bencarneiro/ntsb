...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Person.objects.filter(accident__year=2018).delete()
        person_model_fields = [
            'person_number',
            'age',
            'sex',
        ...
        csv = pd.read_csv(f"{CSV_PATH}2018/person.csv", encoding='latin-1', low_memory=False).fillna(0)
        for x in csv.index:
            ...
            while len(per_no) < 3:
                per_no = "0" + per_no
            primary_key = f"2018{st_case}{veh_no}{per_no}"
            ...
            for model_field_name in person_model_fields:
                data_source = get_data_source("person." + model_field_name, 2018)
                if data_source:
            ...
            data_to_save['helmet_misuse'] = helmet_misuse_converter(csv['REST_USE'][x], csv['REST_MIS'][x], None, 2018)
