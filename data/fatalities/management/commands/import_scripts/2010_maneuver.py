...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Maneuver.objects.filter(vehicle__accident__year=2010).delete()
        csv = pd.read_csv(f"{CSV_PATH}2010/FARS2010NationalCSV/MANEUVER.CSV", encoding='latin-1')
        for x in csv.index:
            ...
            primary_key = f"2010{st_case}{veh_no}{new_maneuver_id}"
            data_to_save = {"vehicle": vehicle, "id": primary_key}
            data_source = get_data_source("maneuver.driver_maneuvered_to_avoid", 2010)
            csv_field_name = data_source.split(".")[1]
            data_to_save['driver_maneuvered_to_avoid'] = maneuver_converter(csv[csv_field_name][x], 2010)
            print(data_to_save)
