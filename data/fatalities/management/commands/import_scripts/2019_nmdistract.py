...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        NonmotoristDistracted.objects.filter(person__accident__year=2019).delete()
        csv = pd.read_csv(f"{CSV_PATH}2019/FARS2019NationalCSV/NMDistract.CSV", encoding='latin-1')
        for x in csv.index:
            ...
            primary_key = f"2019{st_case}{veh_no}{per_no}{new_distraction_id}"
            data_to_save = {"id": primary_key,"person": person}
            data_source = get_data_source("nonmotorist_distracted.nonmotorist_distracted_by", 2019)
