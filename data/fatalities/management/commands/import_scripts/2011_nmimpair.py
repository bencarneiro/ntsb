...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        NonmotoristImpaired.objects.filter(person__accident__year=2011).delete()
        csv = pd.read_csv(f"{CSV_PATH}2011/FARS2011NationalCSV/NMIMPAIR.CSV", encoding='latin-1')
        for x in csv.index:
            ...
            primary_key = f"2011{st_case}{veh_no}{per_no}{new_impairment_id}"
            
            data_to_save = {"id": primary_key, "person": person}
            data_source = get_data_source("nonmotorist_impaired.nonmotorist_impaired", 2011)
            csv_field_name = data_source.split(".")[1]
            data_to_save['nonmotorist_impaired'] = nonmotorist_impaired_converter(csv[csv_field_name][x], 2011)
            print(data_to_save)
