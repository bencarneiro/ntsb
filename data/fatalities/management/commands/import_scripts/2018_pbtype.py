...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        PedestrianType.objects.filter(person__accident__year=2018).delete()
        model_fields = [
            'age',
            'sex',
            'person_type',
        ...
        csv = pd.read_csv(f"{CSV_PATH}2018/PBType.csv", encoding='latin-1')
        for x in csv.index:
            ...
            for model_field_name in model_fields:
                data_source = get_data_source("pedestrian_type." + model_field_name, 2018)
                if data_source:
