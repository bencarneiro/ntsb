...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        PersonRelatedFactor.objects.filter(person__accident__year=2018).delete()
        csv = pd.read_csv(f"{CSV_PATH}2018/person.csv", encoding='latin-1')
        for x in csv.index:
            if csv['VEH_NO'][x] == 0:
            ...
            for factor in ["P_SF1", "P_SF2", "P_SF3"]:
                ...
                primary_key = f"2018{st_case}{veh_no}{per_no}{new_factor_id}"
                
                data_to_save = {"id": primary_key, "person": person}
                # data_source = get_data_source("person_related_factor.person_related_factor", 2018)
                # csv_field_name = data_source.split(".")[1]
                factor_code = person_related_factor_converter(csv[factor][x], 2018)
                if factor_code:
