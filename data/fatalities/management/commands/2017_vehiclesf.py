...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        VehicleRelatedFactor.objects.filter(vehicle__accident__year=2017).delete()
        ...
        for x in csv.index:
            ...
            for factor in ["VEH_SC1", "VEH_SC2"]:
                ...
                # data_source = get_data_source("vehicle_related_factor.vehicle_related_factor", 2017)
                # csv_field_name = data_source.split(".")[1]
                factor_code = vehicle_related_factor_converter(csv[factor][x], 2017)
                if factor_code:
                    data_to_save = {
                        "id": primary_key,
                        "vehicle": vehicle,
