...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        PersonRelatedFactor.objects.filter(person__accident__year=2012).delete()
        ...
        for x in csv.index:
            ...
            for factor in ["P_SF1", "P_SF2", "P_SF3"]:
                ...
                # data_source = get_data_source("person_related_factor.person_related_factor", 2012)
                # csv_field_name = data_source.split(".")[1]
                factor_code = person_related_factor_converter(csv[factor][x], 2012)
                if factor_code:
