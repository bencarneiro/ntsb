...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Drugs.objects.filter(person__accident__year=2020).delete()
        csv = pd.read_csv(f"{CSV_PATH}2020/FARS2020NationalCSV/drugs.csv", encoding='latin-1')
        for x in csv.index:
            if csv['VEH_NO'][x] == 0:
            ...
            primary_key = f"2020{st_case}{veh_no}{per_no}{new_drugs_id}"
            
            data_to_save = {"id": primary_key, "person": person}
            data_to_save['drug_test_type'] = drug_test_type_converter(csv[get_data_source("drugs.drug_test_type", 2020).split(".")[1]][x], 2020)
            data_to_save['drug_test_results'] = csv[get_data_source("drugs.drug_test_results", 2020).split(".")[1]][x]
            print(data_to_save)
