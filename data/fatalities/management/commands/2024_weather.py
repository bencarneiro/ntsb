...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Weather.objects.filter(accident__year=2024).delete()
        csv = pd.read_csv(f"{CSV_PATH}2024/FARS2024NationalCSV/weather.csv", encoding='latin-1')
        for x in csv.index:
            ...
            primary_key = f"2024{st_case}{new_weather_id}"
