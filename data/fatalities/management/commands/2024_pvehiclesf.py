...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        ParkedVehicleRelatedFactor.objects.filter(parked_vehicle__accident__year=2024).delete()
        csv = pd.read_csv(f"{CSV_PATH}2024/FARS2024NationalCSV/pvehiclesf.csv", encoding='latin-1')
        for x in csv.index:
            print(csv['ST_CASE'][x])
            print(csv['VEH_NO'][x])
            parked_vehicle = ParkedVehicle.objects.get(accident__year=2024, accident__st_case=csv['ST_CASE'][x], vehicle_number=csv['VEH_NO'][x])
            ...
            primary_key = f"2024{st_case}{veh_no}{new_factor_id}"
            data_to_save = {
                "id": primary_key,
