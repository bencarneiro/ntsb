...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        VehicleRelatedFactor.objects.filter(vehicle__accident__year=2006).delete()
        ...
        for x in csv.index:
            ...
            for factor in ["VEH_CF1", "VEH_CF2"]:
                ...
                # data_source = get_data_source("vehicle_related_factor.vehicle_related_factor", 2006)
                # csv_field_name = data_source.split(".")[1]
                factor_code = vehicle_related_factor_converter(csv[factor][x], 2006)
                if factor_code:
                    data_to_save = {
                        "id": primary_key,
                        "vehicle": vehicle,
