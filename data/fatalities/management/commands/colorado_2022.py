...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        InjuryPerson.objects.filter(injury_accident__state_id=8, injury_accident__dt__year=2022).delete()
        InjuryVehicle.objects.filter(injury_accident__state_id=8, injury_accident__dt__year=2022).delete()
        InjuryAccident.objects.filter(state_id=8, dt__year=2022).delete()
        data = pd.read_excel(f"{COLORADO_PATH}CDOTRM_CD_Crash_Listing_-_2022.xlsx")
