...
# multiple sources before 1990

class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        VehicleRelatedFactor.objects.filter(vehicle__accident__year=1990).delete()
        ...
        for x in csv.index:
            ...
            for factor in ["VEH_CF1", "VEH_CF2"]:
                ...
                primary_key = f"1990{st_case}{veh_no}{new_factor_id}"
                # data_source = get_data_source("vehicle_related_factor.vehicle_related_factor", 1990)
                # csv_field_name = data_source.split(".")[1]
                factor_code = vehicle_related_factor_converter(csv[factor][x], 1990)
                if factor_code:
