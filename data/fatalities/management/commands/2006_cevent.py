...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        VehicleSequenceOfEvents.objects.filter(vehicle__accident__year=2006).delete()
        VehicleSequenceOfEvents.objects.filter(parked_vehicle__accident__year=2006).delete()

        # csv = pd.read_csv(f"{CSV_PATH}2006/FARS2006NationalCSV/CEVENT.CSV", encoding='latin-1')
        ...
        for x in csv.index:
            ...
            for event in ['SEQ1', "SEQ2", "SEQ3", "SEQ4", "SEQ5", "SEQ6"]:
                ...
                primary_key = f"2006{st_case}{veh_no}{new_event_id}"
                soe = soe_converter(csv[event][x], 2006)
                data_to_save = {
                    "id": primary_key,
                    "vehicle": vehicle,
        ...
        # csv = pd.read_csv(f"{CSV_PATH}2006/FARS2006NationalCSV/CEVENT.CSV", encoding='latin-1')
        csv = pd.read_csv(f"{CSV_PATH}2006/FARS2006NationalCSV/VEHNIT.CSV", encoding='latin-1')
        for x in csv.index:
            veh_no = str(csv['VEH_NO'][x])
            while len(veh_no) < 3:
                veh_no = "0" + veh_no

            parked_vehicle = ParkedVehicle.objects.get(accident__year=2006, accident__st_case=csv['ST_CASE'][x], vehicle_number=csv['VEH_NO'][x])
            for event in ['SEQ1', "SEQ2", "SEQ3", "SEQ4", "SEQ5", "SEQ6"]:
                st_case = str(csv['ST_CASE'][x])
                if len(st_case) == 5:
                    st_case = "0" + st_case
