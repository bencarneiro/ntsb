...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        Drugs.objects.filter(person__accident__year=2000).delete()
        ...
        for x in csv.index:
            ...
            if csv['VEH_NO'][x] == 0:
                ...
                person = Person.objects.get(Q(accident__year=2000), Q(person_number=csv['PER_NO'][x]),  Q(accident__st_case=csv['ST_CASE'][x]), Q(vehicle__vehicle_number=csv['VEH_NO'][x])|Q(parked_vehicle__vehicle_number=csv['VEH_NO'][x]))
            ...
            for i in [1,2,3]:
                ...
                primary_key = f"2000{st_case}{veh_no}{per_no}{new_drugs_id}"
                # data_to_save = {"id": primary_key, "person": person}
                # data_to_save['drug_test_type'] = drug_test_type_converter(csv[drug_test_type_variable_name][x], 2000)
                ...
                new_drug_object = Drugs(id=primary_key, 
                                        person=person, 
                                        drug_test_type=drug_test_type_converter(csv[drug_test_type_variable_name][x], 2000), 
