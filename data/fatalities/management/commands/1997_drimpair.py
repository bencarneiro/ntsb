...
class Command(BaseCommand):
    def handle(self, *args, **kwasrgs):
        DriverImpaired.objects.filter(vehicle__accident__year=1997).delete()
        ...
        for x in csv.index:
            ...
            for factor in ["DR_CF1", "DR_CF2", "DR_CF3", "DR_CF4"]:
                ...
                primary_key = f"1997{st_case}{veh_no}{new_impairment_id}"

                impairment = driver_impaired_converter(csv[factor][x], 1997)
                if impairment:
